...
func (m *SSHManager) Connect() error {
	...
	if m.PrivateKey != "" {
		...
		if err != nil {
			// fallback/retry mechanisms could go here
...
func (m *SSHManager) RunSudoCommand(command string) (string, string, int) {
	...
	if m.Password != "" {
		escapedPass := strings.ReplaceAll(m.Password, "'", "'\\''")
		fullCmd := fmt.Sprintf("echo '%s' | sudo -S -p '' %s", escapedPass, cleanCmd)
...
func (m *SSHManager) RunSudoScript(script string) (string, string, int) {
	...
	if m.Password != "" {
		escapedPass := strings.ReplaceAll(m.Password, "'", "'\\''")
		fullCmd = fmt.Sprintf("echo '%s' | sudo -S -p '' bash %s; rm -f %s", escapedPass, tmpScript, tmpScript)
	} else {
...
func (m *SSHManager) UploadFileSudo(content, remotePath string) error {
	...
	m.RunSudoCommand(fmt.Sprintf("mv %s %s", tmpName, remotePath))
	m.RunSudoCommand(fmt.Sprintf("chmod 644 %s", remotePath))
